...
const StoreFetch = ({ data, setOpen2 }) => {
	return (
...

				{
					data.map((d, i) => (
						<tr key={i} className="bg-white border-b dark:bg-[#0D0D0D] dark:border-gray-700 hover:bg-gray-50 ">
							...
							<td className="px-6 py-4">
								<Link href={`/main/store?id=${d?.userid._id}`} onClick={() => setOpen2(true)} className="font-medium bg-[#41A956]/30 p-2 px-5 rounded-3xl text-[#41A956] hover:underline">View</Link>
							</td>
						</tr>
					))
				}
