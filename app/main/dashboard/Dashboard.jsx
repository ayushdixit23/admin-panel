    ...
    try {
      ...
      if (res.data.success) {
        setCount({
          ...
          totalCommunities: res.data.community,
        ...
        <div className="mt-4 dark:bg-[#101010] light:text-black rounded-xl p-4">
          ...
          <div className="mt-4">
            <div className="overflow-auto no-scrollbar">
              ...
              {data.latestUsers?.length > postPerPage && (
                <Pagination
        ...
        <div className="mt-4 dark:bg-[#101010] p-4">
          ...
          <div className="w-full flex pp:flex-row flex-col items-center mt-2 gap-4">
            <div
              ...
              className={`${
                switcher == 1
                  ? "bg-[#4495FF] text-white"
                  : "border-[#E0E3E5] border"
              } dark:text-white pn:max-pp:w-full font-bold text-sm p-2 px-5 rounded-xl`}
            >
              Store verification
            </div>
            <div
                ...
                <table className="w-full text-sm text-left min-w-[1100px] rtl:text-right text-gray-500 dark:text-gray-400">
                  ...
                  {data.store.length > 0 && (
                    <tbody>
                      {data.store.map((d, i) => (
                        ...
                        >
                          <td className="px-6 py-4">{d?.userid._id}</td>
                          <th
                            scope="row"
                          ...
                          <td className="px-6 py-4">
                            <Link
                              href={`/main/dashboard?id=${d?.userid._id}`}
                  ...
                  {data.store.length === 0 && (
                    <tr className="w-full light:border-2 light:border-[#f1f1f1]">
                      <td
                        colSpan={5}
                        className=" min-w-full text-2xl font-bold"
                      >
            ...
            {switcher == 2 && (
              <div className="overflow-x-scroll no-scrollbar">
                <table className="w-full text-sm text-left min-w-[1100px] rtl:text-right text-gray-500 dark:text-gray-400">
                  ...
                  {data.product.filter((f) => f?.products?.length != 0).length >
                    0 && (
                    <tbody>
                      {data.product
                        ...
                        .map((d, i) => (
                          ...
                          >
                            ...
                            <td className="px-6 py-4">{d?.products.length}</td>
                            <td className="px-6 py-4">
                              <Link
                                href={`/main/dashboard?id=${d?.userid._id}`}
                  ...
                  {data.product.filter((f) => f?.products?.length != 0)
                    .length === 0 && (
                    <tr className="w-full">
                      <td
                        colSpan={5}
                    ...
                    <tbody>
                      {data.community.map((d, i) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">{d?.topics}</td>

                          <td className="px-6 py-4">{d?.posts}</td>
                          <td className="px-6 py-4">{d?.members}</td>
                          <td className="px-6 py-4">{d?.engagement}</td>

                          <td className="px-6 py-4">{d?.category}</td>
                          <td className="px-6 py-4">
                            <Link
                              href={`/main/dashboard?id=${d?.id}`}
          ...
          <div className="flex justify-end mt-5 items-center">
            {switcher === 1 && data.store.length > 0 && (
              <Link
                className="bg-blue-500 p-2 px-4 rounded-xl  flex justify-end items-center"
                href={"/main/store"}
              >
                See More...
              </Link>
            ...
            {switcher === 3 && data.community.length > 0 && (
              <Link
                className="bg-blue-500 p-2 px-4 rounded-xl  flex justify-end items-center"
                href={"/main/community"}
              >
